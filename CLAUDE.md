...
**The card order is generated too.** `build-ecosystem-order.js` sorts the cards in the `tools-grid` by a maturity (66%) + activity (34%) score — downloads alone are 30%, GitHub stars 10% — computed from `data/stats.json`, `data/downloads.json` and `data/release-notes/*.json`, and writes the same order into `index.html` and all four `<lang>/index.html` copies. Cards are keyed by `data-i18n="tools.items.<key>.title"`, so every card needs that attribute. Fixed positions live in `PINNED`: IFCX at 1 (it is the data format, not an application, and carries the amber border) and Y-app at 6 (a private repo, so too little public data for a fair score); the other cards fill the remaining slots by score, and cards with no measurable data land at the bottom. Scores are computed against the timestamp inside the data files, not the wall clock, so re-running without fresh data is a no-op. Hand-editing the order is pointless — the next pipeline run undoes it. Change `PINNED`, `WEIGHTS` or `SKIP_DOWNLOADS` in the script instead. `SKIP_DOWNLOADS` holds Y-app: it is a private repo, so its download count only reflects people with repo access and the daily workflow cannot read it at all (404). While Y-app is pinned this only affects its printed score, not its position. Missing data drops out of the weighting rather than scoring zero, so a card whose repo is absent from `TOOL_REPOS` in `generate-downloads.js` is shielded from the download weight and floats up — keep that list complete for every homepage card.
...
- `404.html` is the error page. It is `noindex`, deliberately has no `data-i18n="…"` attributes (so `build-i18n-pages.js` skips it and it stays out of the sitemap) and translates through inline `data-i18n-<lang>` attributes like the nav. `build-schema.js` skips every `noindex` page. It only takes effect once nginx has `error_page 404 /404.html;` (`deploy/error-pages.conf`, server config outside this repo). All its paths must stay absolute.
- Favicons: `/favicon.ico` (16/32/48 PNG-in-ICO) and `/apple-touch-icon.png` (180×180) are rendered from `shared/assets/logo/openaec-symbol-v2-transparent.svg`; every page links both next to the SVG icon.
- On mobile, `shared/style.css` gives tables a `min-width: 600px` only inside a scroll container (`.table-wrapper`, `.table-scroll`, `.comparison-wrap` or an inline `overflow-x`). Put a wide table in one of those; a bare table must fit the screen.
...
- `nijmegen/` is an **unlisted** page (a static 3D-viewer build): `noindex` meta, no inbound links, not in `sitemap.xml`, and listed in the `SKIP_DIRS` of every page-scanning script (`build-i18n-pages`, `build-nav-static`, `build-schema`, `build-release-notes-static`, `build-figures`). Keep it that way — a missing skip would translate it, add it to the sitemap or inject nav. Its `model/core.glb` is stored as `core.glb.partNN` (<45 MiB each, GitHub rejects files >100 MB); an inline fetch shim in `nijmegen/index.html` streams the parts back as one response. Re-split and update the shim's `size`/`parts` when the model changes. The compiled viewer bundle is patched so planting is off on load (`vegetation:!1` in the default layer object `const kh={…}` in `assets/index-*.js`); reapply that after dropping in a new viewer build.
